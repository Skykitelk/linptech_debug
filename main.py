...
class Page(tk.Frame):
	def __init__(self, parent,root):
		...
		self.currentPort=tk.StringVar(self)
		self.currentPort.set(self.portNameList[0])
		
		#串口
		self.serial = SerialHelp.SerialHelper(self.currentPort.get())
		self.serial.thresholdValue = 16
		self.createWidgets()
	...
	def createWidgets(self):
		...
		ttk.Label(serial_lf,text="选择串口:").grid(row=0,column=1)

		def changeSignalPort(event):
			if self.serial.alive:
				self.serial.stop()
			self.serial.port = self.currentPort.get()

		ttk.OptionMenu(serial_lf,self.currentPort,*self.portNameList,command=changeSignalPort).grid(row=0,column=2,padx=20)
		ttk.Button(serial_lf,text="打开串口",command=self.serial_open).grid(row=0,column=3)
